...
if os.path.exists('./hashtags_match.txt'):
    with open('./hashtags_match.txt', 'r') as readfile:

#if os.path.exists('./bad_char.txt'):
#    with open('./bad_char.txt', 'r') as readfile:
...
class StreamListener(tweepy.StreamListener):
    ...
    def on_data(self, data):
        ...
        try:
                            ...
                            if rt_num_hashtags != 0:
                                for i in range(rt_num_hashtags):
                                    if re.match(r'.*(vote|midterm|november|election).*',
                                        ...
                                        with open('../mona_hashtags_stream_match.csv', 'a+') as f:
                                            csv_writer1 =csv.writer(f, delimiter = ',')
                                            csv_writer1.writerow([
                                                datajson['retweeted_status']['extended_tweet']['entities']['hashtags'][
                                                    i]['text'].encode('utf-8')])
                                    else:
                                        with open('../mona_hashtags_stream_not_match.csv', 'a+') as f:
            ...
            if os.path.exists('../mona_hashtags_stream_match.csv'):
                with open('../mona_hashtags_stream_match.csv', 'r') as readfile:
                    res1 = readfile.read().split('\n')
                ...
                for k, v in mycounter.most_common():
                    with open('../mona_stream_result_matchtmp.csv', 'a+') as outfile:
                        csvWriter1 = csv.writer(outfile, delimiter = ',')
                        csvWriter1.writerow([k.encode('utf-8'),v])
                move('../mona_stream_result_matchtmp.csv', '../mona_stream_result_match.csv')

            if os.path.exists('../mona_hashtags_stream_not_match.csv'):
                with open('../mona_hashtags_stream_not_match.csv', 'r') as readfile:
                ...
                for k, v in mycounter.most_common():
                    with open('../mona_stream_result_not_matchtmp.csv', 'a+') as outfile:
                        csvWriter2 = csv.writer(outfile, delimiter = ',')
                        csvWriter2.writerow([k.encode('utf-8'),v])
                move('../mona_stream_result_not_matchtmp.csv', '../mona_stream_result_not_match.csv')
